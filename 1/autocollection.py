...
def autocollection(driver,z_name):
    ...
    ceshi_date ="2022/"+re_compile.findall(element.text)[0].replace("Date:2022/", "").strip()
    # 获取车辆
    ...
    if body_element:
        ...
        for tag in tags:
            ...
            try:
                ...
                comment_info = body_element_re_compile.findall(body_text)[0].strip().replace(tag.text,"").replace("(","").replace(")","")
                try:
                    shuzi_re_compile = re.compile(r"[0-9]+")
                    shuzi = shuzi_re_compile.findall(comment_info)[0].strip()
                    comment_info = comment_info.replace(shuzi,"")
                    if comment_info == "":
                        comment_info = tag.text.split("/")[-2] + "/" + tag.text.split("/")[-1]
                except Exception as e:
                    print(e)
            ...
            info_list = [upload_people,branch,commit,ceshi_date,module,comment_info,anser_descrip,level,xray_link,xray_link_time,vehicle,]
            data.append(info_list)

        df = pd.DataFrame(data=data,
                          columns=["triager","分支名","commit","测试日期","模块名","描述信息","tag","所在等级","xray链接","时间段", "车辆"
                                   ])

        df.to_csv('data_1212.csv', mode="a+", header=False, index=False)
